...
def cal_cagr(df, start_date, end_date, hold_num, min, max, rank_factors, enable_stop_profit=True):
    ...
    if threadhold_num:
        ...
        for trade_date, _df in df.groupby('trade_date'):
            ...
            _ranks_df['mod_rank'] = (_ranks_df['ori_rank'] - threadhold_num).where(_ranks_df['last_rank'] <= hold_num,_ranks_df['ori_rank'])
    ...
    try:
        ...
        print(f"{'止盈' if enable_stop_profit else '不止盈'} - QuantStats CAGR (periods=365): {cagr_quantstats_365:.6f} ({cagr_quantstats_365*100:.2f}%)")
        print(f"{'止盈' if enable_stop_profit else '不止盈'} - QuantStats CAGR (periods=252): {cagr_quantstats_252:.6f} ({cagr_quantstats_252*100:.2f}%)")
...
if __name__ == '__main__':
    ...
    print("计算不止盈情况的CAGR")
    print("=" * 30)
    cagrs_no_stop, bonds_no_stop, returns_no_stop = cal_cagr(df, start_date, end_date, hold_num, min, max, factors, enable_stop_profit=False)
    
    print("\n" + "=" * 30)
    print("计算启用止盈情况的CAGR")
    print("=" * 30)
    cagrs_with_stop, bonds_with_stop, returns_with_stop = cal_cagr(df, start_date, end_date, hold_num, min, max, factors, enable_stop_profit=True)
    ...
    if closest_method_no_stop:
        ...
        print(f"   差值: {closest_diff_no_stop*100:.2f}%点，计算结果: {cagrs_no_stop[closest_method_no_stop]*100:.2f}%")
    ...
    if closest_method_with_stop:
        print(f"2. 启用止盈情况 - 与lude.cc结果({lude_cagr_with_stop*100:.2f}%)最接近的方法: {closest_method_with_stop}")
        print(f"   差值: {closest_diff_with_stop*100:.2f}%点，计算结果: {cagrs_with_stop[closest_method_with_stop]*100:.2f}%")
