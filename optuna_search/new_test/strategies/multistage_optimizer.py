...
def multistage_optimization(df, factors, num_factors, args, max_combinations=50000):
    ...
    storage_name = f"sqlite:///{first_stage_study_name}.db"
    ...
    def first_stage_objective(trial):
        ...
        try:
            cagr = cal_cagr(
                    ...
                    threshold_num=None,
                    min=args.price_min if args else 100,
                    max=args.price_max if args else 150,
    ...
    second_storage_name = f"sqlite:///{second_stage_study_name}.db"
    ...
    final_storage_name = f"sqlite:///{final_study_name}.db"
    ...
    if abs(value_diff) < 0.0001:
        ...
        try:
            ...
            # 为每个参数创建适当的分布
            if 'combination_idx' in first_stage_best_params:
                distributions['combination_idx'] = optuna.distributions.IntDistribution(0, len(first_stage_combinations) - 1)
    ...
    if use_second_stage:
        ...
        try:
            if hasattr(second_stage_study.best_trial, 'user_attrs') and 'rank_factors' in second_stage_study.best_trial.user_attrs:
                # 直接从user_attrs获取
        ...
        try:
            ...
            # 确保最佳试验的索引被正确更新
            if final_study.best_trial.number != len(final_study.trials) - 1 and final_study.best_value <= second_stage_best_value:
                print("更新最终研究的最佳试验索引")
                ...
                if hasattr(final_study.best_trial, 'user_attrs') and 'rank_factors' in final_study.best_trial.user_attrs:
