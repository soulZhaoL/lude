...
def cal_cagr(df, start_date, end_date, hold_num, threshold_num, min, max, rank_factors):
    ...
    # df.loc[df.remain_size < 1000, 'filter'] = True # 排除剩余规模
    ...
    # df.loc[df.redeem_remain_days < 1, 'filter'] = True # 强制赎回天数小于1 (不存在的,无效因子)
...
if __name__ == '__main__':
    ...
    factors = [{'name': 'dblow', 'weight': 4, 'ascending': False}, {'name': 'volatility_stk', 'weight': 3, 'ascending': True}, {'name': 'remain_size', 'weight': 4, 'ascending': False}]
    cagr = cal_cagr(df, start_date, end_date, 5, None, 100, 200, factors)
    print(cagr)
